...
std::string QNode::showMaster() { return ros::master::getURI(); }
...
template <class T> std::string QNode::timeToStr(T ros_t) {
  (void)ros_t;
...
bool QNode::checkDisk() {
  ...
  if (free < options_.min_space) {
    echoError("%f GB of free space is available on disk with %s.  Disabling "
              "recording.",
              free_gb, bag_.getFileName().c_str());
    writing_enabled_ = false;
    return false;
  } else if (free < 5 * options_.min_space) {
    ROS_ERROR("%f GB of free space is available on disk with %s.", free_gb,
  ...
  boost::filesystem::path p(
      boost::filesystem::system_complete(bag_name.c_str()));
  ...
  if (info.available < options_.min_space) {
    writing_enabled_ = false;
    throw rosbag::BagException(std::to_string(free_gb) +
                               "GB of free space is available on disk with " +
                               bag_.getFileName() + ". Disabling recording.");
...
void QNode::checkNumSplits() {
  if (options_.max_splits > 0) {
    ...
    if (current_files_.size() > options_.max_splits) {
      ...
      if (err != 0) {
        ROS_ERROR("Unable to remove %s: %s", current_files_.front().c_str(),
                  strerror(errno));
      }
...
bool QNode::checkLogging() {
  ...
  if (now >= warn_next_) {
    ...
    ROS_ERROR(
        "Not logging message because logging disabled.  Most likely cause "
        "is a full disk.");
  }
  return false;
}
...
QString QNode::formatFilenames(QString filename) {
  ...
  if (parts.size() == 0) {
    ROS_ERROR("Bag filename is empty (neither of these was specified: prefix, "
              "append_date, split)");
  }
