...
MainWindow::MainWindow(int argc, char **argv, QWidget *parent)
    : QMainWindow(parent), qnode(argc, argv), file_name("/rosbag.bag"),
  ...
  if (!init_ros_ok) {
    ...
    ui.label_master->setText("Connected to master at:" +
                             QString::fromStdString(qnode.showMaster()));
    RecordState = Stopped;
  }
...
void MainWindow::updateUI() {
  switch (RecordState) {
  case Unstarted:
    ui.logging_status_flag->setText(
...
void MainWindow::on_button_toggle_recording_clicked(bool check) {
  ...
  case Running:
    ...
    if (qnode.showBagSize().isNull()) {
      ...
      ui.save_flag->setText("Unsaved recorder data. Size is " +
                            qnode.showBagSize());
...
void MainWindow::updateTopics() {
  ...
  for (const auto &sub : subscription) {
    QList<QListWidgetItem *> disp_topics =
        ui.list_topics->findItems(sub, Qt::MatchContains);
...
void MainWindow::on_button_subscribe_clicked(bool check) {
  ...
  ui.topic_counter->setText(QString::number(add_topics.size()) +
...
void MainWindow::on_button_unsubscribe_clicked(bool check) {
  ...
  ui.topic_counter->setText(QString::number(rm_topics.size()) +
                            QString(" topics selected."));
